<template>
   <div class="menu_left cflex" :style="{width:sidebar.width+'px'}">
        <div class="menu_page_top rflex">
            <img :class='["logo",{"closeLogo":!sidebar.opened}]' :src="logo" alt="shopAdmin" >
            <span class='title' v-show="sidebar.opened">商铺<i>Admin</i></span>
        </div>
        <div class="menu_page_bottom is-scroll-left">
            <el-menu 
                ...
                >
                    <template v-for="(item,index) in tyr">
                        <!--表示 有一级菜单-->
...
export default {
  ...
  computed:{
      ...
      tyr(){return this.$store.state.permission.routers}
  },
  created(){
  },
  mounted(){
